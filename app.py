...
def is_valid_img_path(im_path):
    ...
    return (os.path.exists(im_path)
            ...
            and os.path.splitext(im_path)[1].lower() in [".jpeg", ".jpg"])  # check if file is a jpeg image
...
def format_byte_count(total_bytes):
    ...
    # store the power of 1024 which is nearest to total bytes, the power will be the index of size_name
...
class App:
    def __init__(self) -> None:
        ...
        self.drag_delta = (0, 0)  # stores the amount mouse was dragged in x-axis and y-axis
        ...
        # save button
        self.save_btn = Button(self.save_btn_bg, self.save_btn_hover_bg, save_btn_text_color, self.font,
                               "Save",
                               (80, 40), (0, 0))
        ...
        self.resolution_slider = Slider(slider_bg, slider_fg, slider_text_color, self.font,
                                        1, 100, 100,
                                        "Resolution: 0 x 0",
                                        (200, 40), (0, 0))
    ...
    def update_image_quality_and_resolution(self, new_quality, new_resolution):
        """
        This function will use PIL to modify the self.active_img_surface to the quality and resolution set by the
        user using the sliders
        """
        if self.original_img_surface is None:  # if no image has been loaded then return
            return
        ...
        # pil_image is resized to new_resolution  and saved with new_quality if new_resolution has been provided
        if new_resolution is not None:
            pil_img = pil_img.convert("RGB").resize(new_resolution, Image.Resampling.NEAREST)
            self.new_img_res = new_resolution

        # update information regarding image quality by saving it to a buffer in memory and then counting the number
        # of bytes in it the image is saved with quality provided in new_quality
    ...
    def load_img(self, path):
        ...
        try:
            ...
            self.toast.show(f"Loaded {self.original_img_path} {self.modified_img_path}")
    ...
    def handle_event(self, event_data):
        ...
        # ---------------------------------------- MOUSE BUTTON DOWN EVENT ---------------------------------------------
            ...
            # check if right mouse button is pressed
                ...
                # get_rel() gives the change in mouse position since last time it was called as a tuple(dx,dy).
                # So it is called here because pressing right mouse button triggers dragging of image and for that
                # when pygame.mouse.get_rel() gets called in update() method
                # pygame.mouse.get_rel() will give correct change in mouse position
        ...
        # ---------------------------------------- MOUSE BUTTON UP EVENT -----------------------------------------------
            ...
            if self.is_dragging_on_res_slider:
                ...
                new_resolution = (int(self.img_org_res[0] * slider_ratio), int(self.img_org_res[1] * slider_ratio))
                self.update_image_quality_and_resolution(self.quality_slider.value, new_resolution)
        ...
        # ---------------------------------------- MOUSE WHEEL EVENT (ZOOMING) -----------------------------------------
            ...
            # Store the rotation of mouse wheel. It is +ve is scrolling up and -ve values for scrolling down
    ...
    def render(self):
        ...
        if self.active_img_surface is not None:
            ...
            img_border = (self.img_render_pos[0] - border_width, self.img_render_pos[1] - border_width,
